...
def plot_top5_last5(df1,suffix, label1, label2, ylabel, output_dir, csv1_name, model_name,opt_name,tmp_r=10):
    ...
    # 支持任意选择方式：等间隔 or 自定义
    selected_indices = np.linspace(0, 9, 10, dtype=int)
    ...
    #print(class_indices)
    colors = [cm.viridis(idx / 10) for idx in class_indices]  # 颜色映射基于 class index
    ...
    def plot(df, filename_suffix):
        ...
        for i, col in enumerate(selected_cols):
            ...
            # 绘制修改后的数据
            plt.plot(values, color=colors[i], linewidth=2)
...
if __name__ == '__main__':
    ...
    for model_name, optim_dict in cifar_BEST_LRS.items():
        ...
        for optimizer, lr in optim_dict.items():
            ...
            #print(model_short)
            try:
                csv_path = f"log/adapolycifar_r10_{model_short}_{optimizer}_batch256_200e_lr{lr}_seed0_v2.csv"
                print(csv_path)
                plot_loss_and_accuracy(csv_path, 'plots/acc_rmsprop', model_name, optimizer,10)
            except:
                print('error:',csv_path)
                csv_path = f"log/adapolycifar_r10_{model_short}_{optimizer}_batch256_200e_lr{lr}_seed7_v2.csv"
                print(csv_path)
                plot_loss_and_accuracy(csv_path, 'plots/acc_rmsprop', model_name, optimizer,10)
                ...
                #         plot_loss_and_accuracy(csv_path, 'plots/0909_acc_r20', model_name, optimizer,20)
                #     except:
