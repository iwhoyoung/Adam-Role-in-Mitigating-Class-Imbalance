...
from adassd_gamma import Adam_bn,Adam_ini,Adam_Sbn
try:
...
class CategoryLossTracker:
    ...
    def update(self, targets, losses):
        ...
        # 累加到历史统计
        self.category_loss += loss_sum.detach().cpu().numpy()
        self.category_count += count_sum.detach().cpu().numpy()
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    mmap=[[0,1],[1,3],[3,7],[7,15],[15,32],[32,68],[68,141],[141,288],[288,555],[555,1000]]
    # 0 : 1300
    # 1 : 1084
    # 2 : 988
    # 3 : 946
    ...
    for group_id in range(10):
        ...
        for cls_id in sorted_classes[start:end]:
            # print(group_id)
            # a+=math.ceil(class_freqs[cls_id])
    ...
    if args.category_loss and (not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0)):
        ...
        train_csv_path = os.path.join(args.output_dir, 'train_category_loss_'+str(args.lr)+'_'+str(args.batch_size)+'_'+str(args.optimizer)+'_'+str(args.arch)+str(args.scheduler)+'_'+str(args.epochs)+'_v2'+'.csv')
        # 验证集CSV路径（包含总loss和acc）
        val_csv_path = os.path.join(args.output_dir, 'val_category_loss_'+str(args.lr)+'_'+str(args.batch_size)+'_'+str(args.optimizer)+'_'+str(args.arch)+str(args.scheduler)+'_'+str(args.epochs)+'_v2'+'.csv')
    ...
    if args.dummy:
        ...
        if os.path.exists(os.path.join(args.data, 'train')):
            ...
            valdir = os.path.join("/home/wangjzh/adam_optimizer/data/imagenet2012", 'val')
   ...
   #########
    ...
    if args.category_loss and (not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0)):
        ...
        with open(train_csv_path, 'a', newline='') as f:
            ...
            writer.writerow([-1, valid_loss, 0, 0] + val_losses_grouped)
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
            ...
            if args.category_loss:
                ...
                with open(train_csv_path, 'a', newline='') as f:
                    ...
                    writer.writerow([epoch, train_loss, 0, 0] + train_losses_grouped)
