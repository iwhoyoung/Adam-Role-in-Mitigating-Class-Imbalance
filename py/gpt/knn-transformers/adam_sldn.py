...
class Adam_ldn(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9,0.999), eps=1e-8,
        ...
        super(Adam_ldn, self).__init__(params, defaults)


    def __setstate__(self, state):
        super(Adam_ldn, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault('amsgrad', False)
...
class Adam_s(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps,
                        weight_decay=weight_decay, amsgrad=amsgrad)
        self.weight = Parameter(torch.zeros(1,device='cuda'))
        super(Adam_s, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(Adam_s, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault('amsgrad', False)
...
class Adam_s2(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps,
                        weight_decay=weight_decay, amsgrad=amsgrad)
        self.weight = Parameter(torch.zeros(1,device='cuda'))
        super(Adam_s2, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(Adam_s2, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault('amsgrad', False)
    ...
    def adam(self,
        ...
        for i, param in enumerate(params):
            ...
            param.addcdiv_(exp_avg, torch.mean((exp_avg*exp_avg.conj())).sqrt().add_(eps), value=-step_size*initial_norm[i])
...
class Adam_s_ldn(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9,0.999), eps=1e-8,
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps,
                        weight_decay=weight_decay, amsgrad=amsgrad)
        self.weight = Parameter(torch.zeros(1,device='cuda'))
        super(Adam_s_ldn, self).__init__(params, defaults)


    def __setstate__(self, state):
        super(Adam_s_ldn, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault('amsgrad', False)
...
class Adam_s_ldnx(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9,0.999), eps=1e-8,
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps,
                        weight_decay=weight_decay, amsgrad=amsgrad)
        self.weight = Parameter(torch.zeros(1,device='cuda'))
        super(Adam_s_ldnx, self).__init__(params, defaults)


    def __setstate__(self, state):
        super(Adam_s_ldnx, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault('amsgrad', False)
