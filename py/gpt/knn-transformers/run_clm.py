...
from adam_util import Adam_bn,Adam_ini,Adam_Sbn,Sgd_m
import logging
import math
import random
import os
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(log_file, mode="a", encoding="utf-8"),  # ★★ NEW ★★
            logging.StreamHandler(sys.stdout),                         # 继续在终端输出
        ],
        datefmt="%m/%d/%Y %H:%M:%S",
    )
        ...
        if data_args.max_train_samples:
            ...
            raw_datasets = DatasetDict({
                ...
                "validation": load_dataset(
                    data_args.dataset_name,
                    data_args.dataset_config_name,
                    split=f'train[{data_args.max_train_samples}:{data_args.max_train_samples + data_args.max_eval_samples}]',
                    cache_dir=model_args.cache_dir,
                    token=model_args.token,
                    streaming=data_args.streaming,
                    trust_remote_code=model_args.trust_remote_code,
    ...
    def build_token_groups(
        ...
        """
        按 token 出现频次把整个 vocabulary 均分成 `num_groups` 组。
        
        Args:
            datasets_dict (DatasetDict): 例如 `tokenized_datasets`
            num_groups (int): 组数
            col (str): 列名，默认 "input_ids"
            streaming (bool): 如果开了 --streaming，就用 .iter() 方式遍历
        
        Returns:
            token_to_group (dict[int, int])
            group_token_ids (list[torch.Tensor])
            group_stats (list[tuple[int,int,float]]): (组内 token 个数, 组内总 freq, 组内 freq 占比)
        """
        # 1️⃣ 统计 token 频次
        freq = Counter()
        if streaming:
            ...
            for split in datasets_dict:
                ...
                freq.update(chain.from_iterable(split_ids))
        
        # 2️⃣ 频次从高到低排序
        sorted_items = freq.most_common()                     # List[(tok, freq)]
        total_freq = sum(freq.values())
        step = total_freq / num_groups                        # 每组目标累计频次
        
        # 3️⃣ 均分到 num_groups 里
        token_to_group = {}
    ...
    # class GroupLossTrainer(Trainer):
    #     def compute_loss(
    #         self,
    #         model,
    #         inputs,
    ...
    #             shift_labels.view(-1),
    #         ).view(shift_labels.size())           # (bs, seq)
    
    #         # -------- 每组 loss ----------
    #         for gid, gid_tokens in enumerate(group_token_ids):
    #             mask = torch.isin(shift_labels, gid_tokens.to(shift_labels.device))
    #             if mask.any():
    #                 group_loss = token_loss.masked_select(mask).mean()
    #                 # 👇 转成 float，避免 JSON 报错
    #                 self.log({f"group_{gid}_loss": group_loss.item()})
    #             # 如果当前 batch 没这个组，可以选择不 log 或 log 0
    #             # else:
    #             #     self.log({f"group_{gid}_loss": 0.0})
    
    #         # -------- 总 loss ----------
    #         loss = token_loss.mean()
    #         self.log({"total_loss": loss.item()})    # 或者 .detach().cpu().item()
    #         # 这里 Trainer 内部会自己把 loss.item() 放进日志，不用管
    #         return (loss, outputs) if return_outputs else loss
    def compute_metrics(eval_preds):
        """
        eval_preds = (predictions, label_ids)
        predictions.shape == (bs, seq_len, vocab)
        label_ids.shape == (bs, seq_len)
        """
        logits, labels = eval_preds
        device = torch.device("cpu")          # 评估阶段可在 CPU 上算
        ...
        token_loss = torch.nn.functional.cross_entropy(
            ...
            shift_labels.view(-1),
            reduction="none",
        ).view(shift_labels.size())           # (bs, seq_len)
    
        # --------- 统计每组 loss ----------
        metrics = {}
        for gid, gid_tokens in enumerate(group_token_ids):
            mask = torch.isin(shift_labels, gid_tokens)
            if mask.any():
                metrics[f"group_{gid}_loss"] = token_loss.masked_select(mask).mean().item()
    
        # --------- 总体 loss ----------
        metrics["total_loss"] = token_loss.mean().item()
    ...
    if opt_name == 'adam':
        ...
        optimizer = Sgd_m(model.parameters(), training_args.learning_rate, weight_decay=0)
        ...
        optimizer = torch.optim.RMSprop(model.parameters(), training_args.learning_rate, weight_decay=0)
    ...
    # Training
    if training_args.do_train:
        ckpt0 = os.path.join(training_args.output_dir, "checkpoint-0")     # ★★ NEW ★★
