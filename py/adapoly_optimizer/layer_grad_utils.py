...
sys.path.append("/lichenghao/huY/adapoly_optimizer/py")
import timm
import torch
...
def cal_nao_distribution(outer_loader, inner_loader, criterion, model, path, loop_num=100):
    ...
    opt = "adam"
    step = "0"
    save_csv_path = f'layer_nao_distribution_{model_name}_{opt}_epoch{step}.csv'
    save_plot_path = f'layer_nao_plot_{model_name}_{opt}_epoch{step}.png'
    naos=[]
    group_num=10
    ...
    for i, (input, _,target) in enumerate(outer_loader):
        if i >= loop_num:
            break
        input = input.cuda()
        target = target.cuda()
        ...
        for param in model.parameters():
            init_grads.append(param.grad.clone().reshape(-1))
            param.grad.zero_()
        for j, (inner_input,_, inner_target) in enumerate(inner_loader):
            if j<=i:
                continue
            if j >=loop_num:
                break
            ...
            for k,(param) in enumerate(model.parameters()):
                ...
                layer_nao = torch.sum(torch.abs(init_grads[k]/(torch.sum((init_grads[k]*init_grads[k].conj())).sqrt()))*torch.abs(g/torch.sum((g*g.conj())).sqrt()))
                param.grad.zero_()
...
if __name__ == '__main__':
    ...
    # from vit import vit_cifar_patch4_224,vit_small_cifar_patch4_32,vit_custom_cifar_32
    ...
    # model = ResNetOrigin(Bottleneck, [3, 4, 6, 3],num_classes=10)
    ...
    # model = nn.Sequential(ResNetOrigin(BasicBlock, [1, 1, 1, 1],num_classes=10),
                        #   vit_custom_cifar_32(num_classes=10))
    # path = 'model_cifar10_resnet18vconv13x3_adassd_batch256_200e_lr0.01_seed0'
    path = 'model_cifar10_res18vconv13x3_adau_batch256_200e_lr0.01_seed0'
    # path = 'imagenet1k_vits_pretrained_timm'
    # model.load_state_dict(torch.load(account + '/adapoly_optimizer/model/'model_%s.pth'%path),strict=True)

    # model.load_state_dict(torch.load(account + '/assets/common_model/resnet18-5c106cde.pth'))
    # model.load_state_dict(torch.load(account + '/assets/common_model/resnet50-19c8e357.pth'))
    # pretrained_state = torch.load(account + '/hy/asset/common_model/densenet121-a639ec97.pth')
    # # 修改 key
    ...
    # test_data = CIFAR100LT(root="/your/path/to/cifar100-lt",
    # train=False,
    # subset_path="cifar100_lt_test_subset.pkl")
    test_data = CIFAR100LT(root="/your/path/to/cifar100-lt",
    train=True)
    # test_data = CusImgPair(account + '/assets/dataset/adv_imagenet1000/data', train=False, transform=transforms_test)
    # test_data = CusImgPair(account + '/hy/nn_property/adv_mifgsm', train=False, transform=transforms_test)
    # test_data = Food101(root = account + "/hy/asset/dataset", split = "train", transform=transforms_test)
    test_loader = torch.utils.data.DataLoader(test_data, batch_size=1, shuffle=False, num_workers=1)
    test_loader2 = torch.utils.data.DataLoader(test_data, batch_size=1, shuffle=False, num_workers=1)   
    # calculator = OrthCalculator(model.parameters()) 
    # cal_data_grad_orthogonality(test_loader,test_loader2,criterion,model,path)
    # cal_data_grad_orthogonality_v2(test_loader,test_loader2,criterion,model,(account + '/hy/nn_property/submit/%s'%path))
    cal_nao_distribution(test_loader,test_loader2,criterion,model,(account + '/adapoly_optimizer/submit/%s'%path))
    # cal_grad_mean(model,criterion,test_loader,path)
    pass
