...
def load_optimizer_csvs(name_list, label_list, save_path):
    ...
    for file, label in zip(name_list, label_list):
        try:
            ...
            # 4. 安全访问列数据
            if len(df.columns) < 1:
                print(f"⚠️ 文件缺少数据列: {file}")
                continue
            mgos.append(df.iloc[-1])    
            values = df.iloc[:, 0].values  # 取出第1列
            
            # 5. 添加您需要的长度验证 (示例使用11)
            expected_length = 11  # 根据您的实验需求修改
            if len(values) != expected_length:
                print(f"⚠️ 跳过文件（长度不是{expected_length}）：{file}")
                continue
    ...
    print(mgos)
    # print(last_values)
    plt.plot(labels, mgos, marker='o', linewidth=3)
...
if __name__ == '__main__':    
    ...
    #     t_name = each["name"]
    #     for each_opt in ["sgd","adam","adam_bn","adam_ini"]:
    #     #for each_opt in ["sgd"]:
    #         labels = []
    #         file_list=[]
    #         lr_list = lr_neighbourhood(each[each_opt], k=3)
    #         for each_lr in lr_list:
    #             model_path = f"/home/wangjzh/adam_machinism/model/adapoly/cifar_r10_{t_name}_{each_opt}_batch256_lr{each_lr}_seed0_1,1,1,1/final.pth"
    #             log_path=f"/home/wangjzh/adam_optimizer/log/grad/cifar_r10_{t_name}_{each_opt}_batch256_lr{each_lr}_grad.csv"
    #             cal_distr_api(t_name,model_path,log_path) 
    #             file_list.append(log_path)
    #             labels.append(str(each_lr))
    #         merge_csv_path = f"{t_name}_{each_opt}_lr.csv"
    #         heatmap_save_path = f"/home/wangjzh/adam_optimizer/plots/grad_lr/{t_name}_{each_opt}_lr.png"
    #         lineplot_save_path = f"/home/wangjzh/adam_optimizer/plots/grad_lr/{t_name}_{each_opt}_last_column.png"
    
    #         load_optimizer_csvs(file_list, labels, merge_csv_path)
    #         heat_map(merge_csv_path, labels, heatmap_save_path)
    #         plot_last_column_from_heatmap(merge_csv_path, labels, lineplot_save_path)
    #model_path = f"/home/wangjzh/adam_machinism/model/adapoly/cifar_r10_{t_name}_{each_opt}_batch256_lr{each[each_opt]}_seed0_1,1,1,1/epoch{step}.pth"
    # log_path=f"/home/wangjzh/adam_optimizer/log/grad/vitb_ini.csv"
    # cal_distr_api('vitb','',log_path) 
    # log_path=f"/home/wangjzh/adam_optimizer/log/grad/vits_ini.csv"
    # cal_distr_api('vits','',log_path)
    # import sys
    # sys.exit()
    
    for each in ['gpt']: 
        t_name = each
        for each_opt in ["sgd","adam","adam_bn","adam_ini","adam_Sbn"]:
            labels = []
            file_list=[]
            #for each_opt in ["sgd"]:
            #step_list = ['000', '010', '020', '030', '040', '050', '060', '070', '080', '090','100', '110', '120', '130', '140', '150', '160', '170', '180', '190', '200']
            step_list = ['0','5000','10000','15000','20000','25000','30000','35000','40000','45000','50000']
            for step in step_list:
                model_path = f"/home/wangjzh/adam_machinism/model/adapoly/cifar_r10_{t_name}_{each_opt}_batch256_lr{each[each_opt]}_seed0_1,1,1,1/epoch{step}.pth"
                log_path=f"/home/wangjzh/adam_optimizer/py/gpt2/nao_distribution_gpt_{each_opt}_{step}.csv"
                cal_distr_api(t_name,model_path,log_path) 
                file_list.append(log_path)
                labels.append(step)
            
            merge_csv_path =f"{t_name}_{each_opt}_step.csv"
            heatmap_save_path =f"/home/wangjzh/adam_optimizer/plots/grad_step/{t_name}_{each_opt}_step.png"
            lineplot_save_path = f"/home/wangjzh/adam_optimizer/plots/gpt_grad_step/{t_name}_{each_opt}_lst_column.png"
            # labels=['000', '020', '040',  '060',  '080', '100',  '120',  '140',  '160',  '180',  '200']
            labels=['0','5k','10k','15k','20k','25k','30k','35k','40k','45k','50k']
            # load_optimizer_csvs(file_list, labels, merge_csv_path)
            load_optimizer_csvs(file_list, labels, lineplot_save_path)
            #heat_map(merge_csv_path, labels, heatmap_save_path)
            # plot_last_column_from_heatmap(merge_csv_path, labels, lineplot_save_path)
    csv_list=[]
    # optimizers_to_compare = ["sgd", "adam", "adam_bn"]
    # models_to_compare = ["resnet18", "resnet50", "vgg16bn", "vitb", "vits"]
    ...
    # #     for each in BEST_LRS:
    # #         t_name = each["name"]
    # #         lr = each[opt]  # 使用每个模型最佳 lr
    # #         label = t_name
    # #         log_path = f"/home/wangjzh/adam_optimizer/log/grad/cifar_r10_{t_name}_{opt}_batch256_lr{lr}_grad.csv"
    # #         model_path = f"/home/wangjzh/adam_machinism/model/adapoly/cifar_r10_{t_name}_{opt}_batch256_lr{lr}_seed0_1,1,1,1/final.pth"
    ...
    # #     # 保存和绘图
    # #     merge_csv_path = f"compare_models_{opt}.csv"
    # #     csv_list.append(merge_csv_path)
    # #     heatmap_save_path = f"/home/wangjzh/adam_optimizer/plots/grad_compare_models/{opt}_heatmap.png"
    # #     lineplot_save_path = f"/home/wangjzh/adam_optimizer/plots/grad_compare_models/{opt}_last_column.png"

    # #     load_optimizer_csvs(file_list, labels, merge_csv_path)
    # #     heat_map(merge_csv_path, labels, heatmap_save_path)
    # #     plot_last_column_from_heatmap(merge_csv_path, labels, lineplot_save_path)
    # # t_name = 'vgg16bn'
    # # lr = '0.0005'  # 使用每个模型最佳 lr
    # # step=200
    # # label = t_name
    # # opt='adam'
    # # log_path=f"/home/wangjzh/adam_optimizer/log/grad/cifar_r10_{t_name}_{opt}_batch256_epoch{step}_grad.csv"
    # # model_path = f"/home/wangjzh/adam_machinism/model/adapoly/cifar_r10_{t_name}_{opt}_batch256_lr{lr}_seed0_1,1,1,1/final.pth"
    ...
    # # 保存和绘图
    # merge_csv_path = f"compare_models_ori.csv"
    # load_optimizer_csvs(file_list, labels, merge_csv_path)
    # csv_list.append(merge_csv_path)
    # for opt in optimizers_to_compare:
    #     labels = []
    ...
    #     heat_map(merge_csv_path, labels, heatmap_save_path)
    #     plot_last_column_from_heatmap(merge_csv_path, labels, lineplot_save_path)
    # save_path = f"/home/wangjzh/adam_optimizer/plots/grad_compare_models/all_last_column.png"
    # plot_multiple_lines(csv_list,['Init.','SGD','Adam','Adam-LDN'], save_path, x_label='Model', y_label='Gradient Orthogonality')
