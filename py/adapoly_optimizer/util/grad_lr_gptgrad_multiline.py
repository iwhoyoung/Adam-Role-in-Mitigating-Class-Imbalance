...
def plot_last_column_from_heatmap(csv_path, labels, save_path):
    ...
    """
    df = pd.read_csv(csv_path, header=None).T.iloc[-1]  # 与 heat_map 中读取方式完全一致
    ...
    print(labels)
    # print(last_values)
    plt.plot(labels, df, marker='o', linewidth=3)
    ...
    plt.ylabel('Gradient Orthogonality', fontsize=30)
    ...
    for file, label in zip(name_list, label_list):
        try:
            if not os.path.exists(file):
                print(f"⛔ 文件不存在: {file}")
                continue
            df = pd.read_csv(file, header=None, index_col=0 if 'gpt' not in file else None)
            if df.empty:
                print(f"⚠️ 文件内容为空: {file}")
                continue
            if 'gpt' not in file:
                values = df.loc[10].values
            else:
                values = df.iloc[:, 0].values
            data_dict[label] = values
            # print(f"✅ 读取成功: {file}，长度: {len(values)}")
        except pd.errors.EmptyDataError:
            print(f"❌ 文件为空: {file}")

    if not data_dict:
        print(f"⚠️ 未生成任何合并数据: {save_path}")
        return None

    poly_bins = [f"{i*0.1:.1f}" for i in range(11)] if 'gpt' in name_list[0] else ['0.0']
...
def plot_opt_for_model(csv_paths, opt_labels, model_name, save_path, is_gpt=False, x_values=None, x_label='Layer', y_label='Gradient Orthogonality'):
    """
    为每个模型绘制所有优化算法的折线图
    :param csv_paths: list[str]，CSV 文件路径列表，每个对应一个优化算法
    :param opt_labels: list[str]，优化算法标签
    :param model_name: str，模型名称
    :param save_path: 保存图片的路径
    :param is_gpt: bool，是否为GPT模型
    :param x_values: list，x轴值（对于GPT/epoch为step或epoch标签）
    :param x_label: str，x轴标签
    :param y_label: str，y轴标签
    """
    plt.figure(figsize=(10, 6))
    for csv_path, opt in zip(csv_paths, opt_labels):
        df = pd.read_csv(csv_path, index_col=0)
        y_values = df.iloc[:, -1].values
        if is_gpt and opt_labels == 'SGD':
            print(y_values)
        if x_values is not None:
            if is_gpt:
                x_values_plot = np.arange(len(x_values))
            else:
                x_values_plot = np.arange(0, len(x_values) * 20, 20)
        else:
            x_values_plot = np.arange(len(y_values))
            
        plt.plot(x_values_plot, y_values, marker='o', label=opt, linewidth=3)

    plt.xlabel(x_label, fontsize=30)
    plt.ylabel(y_label, fontsize=30)
    if is_gpt:
        plt.xticks(np.arange(len(x_values)), x_values, fontsize=30, rotation=45)
    else:
        plt.xticks(np.arange(0, len(x_values) * 20, 20), x_values, fontsize=30)
...
def load_orthogonality_series(csv_path, is_gpt=False, x_values=None):
    df = pd.read_csv(csv_path, index_col=0)
    if is_gpt:
        ...
        if x_values is not None:
            ...
            x_values_plot = np.arange(0, len(y_values) * 20, 20)
    return x_values_plot, y_values


def plot_models_grid(model_configs, save_path):
    """
    将多个模型的 gradient orthogonality 随 step/epoch 的曲线绘制到 2x3 子图中，带公共图例。
    model_configs: list of dict, each dict contains:
      - title
      - csv_paths
      - opt_labels
      - is_gpt
      ...
      - x_label
    """
    fig, axes = plt.subplots(2, 3, figsize=(22, 11), constrained_layout=False)
    axes = axes.flatten()
    legend_handles = []
    legend_labels = []

    for ax, cfg in zip(axes, model_configs):
        for csv_path, opt in zip(cfg['csv_paths'], cfg['opt_labels']):
            if not os.path.exists(csv_path):
                print(f"⛔ 文件不存在: {csv_path}")
                continue
            x_values_plot, y_values = load_orthogonality_series(csv_path, cfg.get('is_gpt', False), cfg.get('x_values'))
            line, = ax.plot(x_values_plot, y_values, marker='o', label=opt, linewidth=3)
            if opt not in legend_labels:
                legend_handles.append(line)
                legend_labels.append(opt)

        ax.set_title(cfg.get('title', cfg.get('model_name', '')), fontsize=30)
        ax.set_ylim(bottom=0.0, top=0.7)
        ax.set_ylabel('Gradient Orthogonality', fontsize=30)
        ax.grid(True)
        ax.tick_params(axis='both', labelsize=30)

        if cfg.get('is_gpt', False):
            ...
            if total > 6:
                ...
                tick_positions = list(range(total))
            ax.set_xticks(tick_positions)
            ax.set_xticklabels([x_values[i] for i in tick_positions], rotation=45, ha='right', fontsize=30)
            ax.set_xlabel(cfg.get('x_label', 'Step'), fontsize=30)
        else:
            epoch_ticks = np.arange(0, 201, 20)
            if len(epoch_ticks) > 6:
                step = max(1, len(epoch_ticks) // 6)
                tick_positions = epoch_ticks[::step]
                if tick_positions[-1] != epoch_ticks[-1]:
                    tick_positions = np.append(tick_positions, epoch_ticks[-1])
            else:
                tick_positions = epoch_ticks
            ax.set_xticks(tick_positions)
            ax.set_xticklabels([str(int(v)) for v in tick_positions], rotation=45, ha='right', fontsize=30)
            ax.set_xlabel(cfg.get('x_label', 'Epoch'), fontsize=30)
    ...
    fig.legend(
        ...
        ncol=max(1, min(len(legend_labels), 5)),
        fontsize=30,
        frameon=False,
    )
    plt.subplots_adjust(bottom=0.08, top=0.96, left=0.05, right=0.98, hspace=0.6, wspace=0.30)
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    fig.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close(fig)


def create_legend(labels, save_path):
    """
    创建单独的图例图片
    :param labels: list[str]，标签列表
    :param save_path: 保存路径
    """
    fig, ax = plt.subplots(figsize=(10, 2))
    for label in labels:
        ax.plot([], [], label=label, linewidth=3)
    ax.legend(fontsize=60, loc='center', ncol=len(labels))
    ax.axis('off')
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
...
if __name__ == '__main__':    
    ...
    # ??????GPT??????
    t_name = 'gpt'
    opts = ["sgd","adam","adam_bn","adam_ini","adam_Sbn"]
    step_list = ['0','5000','10000','15000','20000','25000','30000','35000','40000','45000','50000']
    labels = ['0','5k','10k','15k','20k','25k','30k','35k','40k','45k','50k']
    csv_paths = []
    for each_opt in opts:
        file_list = [f"/home/wangjzh/adam_optimizer/py/gpt2/nao_distribution_gpt_{each_opt}_{step}.csv" for step in step_list]
        merge_csv_path = f"/home/wangjzh/adam_optimizer/plots/grad_per_model/{t_name}_{each_opt}_step.csv"
        load_optimizer_csvs(file_list, labels, merge_csv_path)
        csv_paths.append(merge_csv_path)
    opt_labels = [opt_display.get(opt, opt) for opt in opts]
    save_path = f"/home/wangjzh/adam_optimizer/plots/grad_per_model/{t_name}_all_opt.png"
    # print(csv_paths)
    plot_opt_for_model(csv_paths, opt_labels, t_name, save_path, is_gpt=True, x_values=labels, x_label='Step')
    model_configs.append({
        'title': model_display['gpt'],
        'csv_paths': csv_paths,
    ...
    epoch_labels = [str(i) for i in range(0, 201, 20)]
    optimizers_to_compare = ["sgd","adam","adam_bn","adam_ini","adam_sbn"]

    for each in BEST_LRS:
        t_name = each["name"]
        csv_paths = []
        opt_labels = []
        for opt in optimizers_to_compare:
            lr = each[opt]
            file_list = [f"/home/wangjzh/adam_optimizer/log/grad/cifar_r10_{t_name}_{opt}_batch256_epoch{epoch:03d}_grad.csv" for epoch in range(0, 201, 20)]
            merge_csv_path = f"/home/wangjzh/adam_optimizer/plots/grad_per_model/{t_name}_{opt}_epoch.csv"
            load_optimizer_csvs(file_list, epoch_labels, merge_csv_path)
            csv_paths.append(merge_csv_path)
            opt_labels.append(opt_display.get(opt, opt))
        save_path = f"/home/wangjzh/adam_optimizer/plots/grad_per_model/{t_name}_all_opt.png"
        plot_opt_for_model(csv_paths, opt_labels, t_name, save_path, is_gpt=False, x_values=epoch_labels, x_label='Epoch')
        model_configs.append({
            'title': model_display[t_name],
            'csv_paths': csv_paths,
            'opt_labels': opt_labels,
            'is_gpt': False,
            'x_values': epoch_labels,
            'x_label': 'Epoch',
        })


    combined_save_path = "/home/wangjzh/adam_optimizer/plots/grad_per_model/all_models_2x3.png"
    plot_models_grid(model_configs, combined_save_path)

    create_legend([opt_display.get(opt, opt) for opt in list(dict.fromkeys(optimizers_to_compare + opts))], "/home/wangjzh/adam_optimizer/plots/legend.png")
