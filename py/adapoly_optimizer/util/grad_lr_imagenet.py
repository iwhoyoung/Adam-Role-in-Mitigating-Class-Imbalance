...
datapath = "/home/wangjzh/adam_optimizer/data/imagenet2012"
...
test_loader = create_loader(val_dataset,  (3, 224, 224), 512)
...
def plot_multiple_lines(csv_paths, label_list, save_path, x_label='X', y_label='Y', title='Multiple Line Plot'):
    ...
    for csv_path, label in zip(csv_paths, label_list):
        try:
            ...
            print(y_values)
            x_values = ["vgg16","resnet18", "resnet50",  "vitb", "vits"]
...
if __name__ == '__main__':    
    imagenet_best_lrs =  [
    {"name":"resnet18","sgd": 0.1,  "adam": 0.001,  "adam_bn": 0.0005, "adam_ini": 0.05},
    {"name":"resnet50","sgd": 0.1,"adam": 0.0005,  "adam_bn": 0.0001, "adam_ini": 0.05},
    {"name":"vgg16bn","sgd": 0.1,  "adam": 0.0001,  "adam_bn": 1e-5, "adam_ini": 0.05},
    {"name":"vitb","sgd": 0.05, "adam": 5e-4,   "adam_bn": 5e-5,  "adam_ini": 0.05},
    {"name":"vits","sgd": 0.1,"adam": 0.001,   "adam_bn": 5e-5,  "adam_ini": 0.05},
    ]
    ...
    #         step_list = ['000', '020', '040',  '060',  '080', '100',  '120',  '140',  '160',  '180',  '200']
    ...
    #             log_path=f"/home/wangjzh/adam_optimizer/log/grad/cifar_r10_{t_name}_{each_opt}_batch256_epoch{step}_grad.csv"
    #             cal_distr_api(t_name,model_path,log_path) 
    #             file_list.append(log_path)
    #             labels.append(step)
    ...
    #         lineplot_save_path = f"/home/wangjzh/adam_optimizer/plots/grad_step/{t_name}_{each_opt}_lst_column.png"
    
    #         load_optimizer_csvs(file_list, labels, merge_csv_path)
    #         heat_map(merge_csv_path, labels, heatmap_save_path)
    #         plot_last_column_from_heatmap(merge_csv_path, labels, lineplot_save_path)
    csv_list=[]
    optimizers_to_compare = ["adam_bn"]
    models_to_compare = ["resnet18"]

    for opt in optimizers_to_compare:
        labels = []
        file_list = []

        for each in imagenet_best_lrs :
            t_name = each["name"]
            if t_name not in models_to_compare:continue
            lr = each[opt]  # 使用每个模型最佳 lr
            label = t_name
            log_path = f"/home/wangjzh/adam_optimizer/log/grad/imagenet_r10_{t_name}_{opt}_batch256_lr{lr}_grad.csv"
            model_path = f"/home/wangjzh/adam_optimizer/py/imagenet/output_{lr}_512_{opt}_{t_name}cosine1/checkpoint.pth.tar"
...
plot_multiple_lines(csv_list,['Trained Model (SGD)','Trained Model (Adam)','Trained Model (Adam (BN))','Trained Model (Adam (LN))','ori'], save_path, x_label='Model', y_label='Gradient Orthogonality')
