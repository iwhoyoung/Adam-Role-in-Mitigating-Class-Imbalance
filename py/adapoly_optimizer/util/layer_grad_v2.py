...
def cal_nao_distribution(outer_loader, inner_loader, criterion, model, path, modelname, loop_num=100):
    naos=[]
    ...
    total_pairs = 0
    # for i, (input, target) in enumerate(outer_loader):
    for i, (input, target) in enumerate(tqdm(outer_loader, desc="Outer Loop")):
        if i >= loop_num:
            break
        input = input.cuda()
        target = target.cuda()
...
if __name__ == '__main__':
    ...
    for constructor, modelname, optimizer, batch, epoch, lr, seed in model_configs:
        ...
        cal_nao_distribution(test_loader, test_loader, criterion, model, log_path,modelname)
