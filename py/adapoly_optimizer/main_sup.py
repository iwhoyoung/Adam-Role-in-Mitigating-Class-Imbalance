...
from util.param_utils import distribution_calculator
...
from vit import vit_cifar_patch4_32, vit_cifar_patch4_32_depth4, vit_cifar_patch4_256, vit_custom_cifar_32, vit_small_cifar_patch4_32, vit_tiny_cifar_patch4_32
...
epoch_num = 60
batch_size = 256
dataset_name = 'custdataset_full'
model_name = 'resnet10vconv13x3'# resnet18vconv13x3 vgg16bn vit-t
opt_name = 'sgd'#sgdwnormandsumadam sgdwredistribution8andnorm
...
if __name__ == '__main__':
    ...
    model = ResNetOrigin(BasicBlock, [1, 1, 1, 1],num_classes=5) #3 4 6 3
    # model = ResNetOrigin(Bottleneck, [3, 4, 6, 3],num_classes=10) #3 4 6 3
    # model = models.resnet18(pretrained=False,num_classes=5) #3 4 6 3
    # model = models.vgg16_bn(pretrained=False,num_classes=1000) #3 4 6 3
    # model = models.vgg16(pretrained=False,num_classes=1000) #3 4 6 3
    # model = models.densenet121() #3 4 6 3
    # model = vit_small_cifar_patch4_32(num_classes=5)
    ...
    # a = torch.load(account + '/ada_optimizer/model/model_icml24_cifar10_resnet18vconv13x3_sgd_batch1024_200e_lr0.400000_seed0.pth')
    # b = torch.load(account + '/ada_optimizer/model/model_icml24_cifar10_resnet18vconv13x3_sgd_batch1024_5i_lr0.400000_seed0.pth')
    # model = DiscResNet(DiscBasicBlock, [2, 2, 2, 2], num_classes=10)
    # model.load_state_dict(torch.load(account + '/ada_optimizer/model/model_cifar10_vgg16bn_adassd_grad_smooth_batch256_200e_lr0.010000_seed0.pth'),strict=False)
    ...
    # get_grad_heatmap(account + "/asset/dataset/17flowers/jpg/3/image_0314.jpg", model, target_class=None,save_path=account + '/ada_optimizer/submit/',
    ...
    # get_guidedgrad_heatmap(account + "/asset/dataset/17flowers/jpg/3/image_0314.jpg", model, target_class=None,save_path=account + '/ada_optimizer/submit/',
    ...
    # cal_sim_norm_model(b, a,account + '/ada_optimizer/submit/sim_icml24_cifar10_resnet18vconv13x3_sgd_batch1024_5i200e_lr0')# model.cuda().state_dict()
    # record_param_name(model.state_dict(),account + '/ada_optimizer/submit/name_%s' % model_name,['running','num'])
    # raise Exception
    # test_cuda()
    if server_mode:
        #'feat1', 'feat2-f', 'feat1-fl', 'feat3-l', 'feat4-fl'
        logger = Visualizer(['feat0-f', 'feat1-f', 'feat2', 'feat3', 'feat4','macc'], 5, log_path=account + '/ada_optimizer/log/',
         name="ada_optimizer",
        ...
        logger = Visualizer(['acc0', 'acc1', 'acc2', 'acc3', 'acc4','macc'], 5, log_path='./', name="Flower17_nusu_resnet18_sup",label='top_nusu_nol2pen_batch2')
    ...
    optimizer = torch.optim.SGD(model.parameters(), lr, weight_decay=0)#1e-6
    ...
    # optimizer = distribution_calculator(model.parameters(), lr, weight_decay=0)#1e-6
    ...
    scheduler =  torch.optim.lr_scheduler.MultiStepLR(optimizer,[45,55],gamma=0.1)#120,160
    # t1 rotate15
    ...
    if use_cuda:
        model = model.cuda()
    train_data = CustomDatasetDivByFile(account + '/ada_optimizer/assets/custdataset1', train=True, transform=transforms_c)#7 4 15 11 6 2 1 3 10 5 0 16 9 12 8 13 14
    test_data = CustomDatasetDivByFile(account + '/ada_optimizer/assets/custdataset1', train=False, transform=transforms_test)#7 4 15 11 6 2 1 3 10 5 0 16 9 12 8 13 14
    test_data0 = CustomDatasetDivByFile(account + '/ada_optimizer/assets/custdataset1', train=False, test_class=0, transform=transforms_test)
    test_data1 = CustomDatasetDivByFile(account + '/ada_optimizer/assets/custdataset1', train=False, test_class=1, transform=transforms_test)
    test_data2 = CustomDatasetDivByFile(account + '/ada_optimizer/assets/custdataset1', train=False, test_class=2, transform=transforms_test)
    test_data3 = CustomDatasetDivByFile(account + '/ada_optimizer/assets/custdataset1', train=False, test_class=3, transform=transforms_test)
    test_data4 = CustomDatasetDivByFile(account + '/ada_optimizer/assets/custdataset1', train=False, test_class=4, transform=transforms_test)
    ...
    # train_data = CIFAR10SimPair(account + '/asset/dataset/cifar10', train=True, transform=transforms_c)#7 4 15 11 6 2 1 3 10 5 0 16 9 12 8 13 14
    # test_data = CIFAR10SimPair(account + '/asset/dataset/cifar10', train=False, transform=transforms_test)
    # test_data = CusImgPair(account + '/ada_optimizer/adv_sample2', train=False, transform=transforms_test)
    # test_data = CusImgPair(account + '/asset/dataset/adv_imagenet1000/data', train=False, transform=transforms_test)
    # train_data = CIFAR100(account + '/asset/dataset/CIFAR100', train=True, transform=transforms_c)
    # test_data = CIFAR100(account + '/asset/dataset/CIFAR100', train=False, transform=transforms_test)
    ...
    # train_data = Food101(root = account + "/asset/dataset", split = "train")
    # test_data = Food101(root = account + "/asset/dataset", split = "test")

    train_loader = torch.utils.data.DataLoader(train_data, batch_size=batch_size, shuffle=True, num_workers=nThreads)
    test_loader = torch.utils.data.DataLoader(test_data, batch_size=256, shuffle=False, num_workers=nThreads)
    test_loader0 = torch.utils.data.DataLoader(test_data0, batch_size=256, shuffle=False, num_workers=nThreads)
    test_loader1 = torch.utils.data.DataLoader(test_data1, batch_size=256, shuffle=False, num_workers=nThreads)
    test_loader2 = torch.utils.data.DataLoader(test_data2, batch_size=256, shuffle=False, num_workers=nThreads)
    test_loader3 = torch.utils.data.DataLoader(test_data3, batch_size=256, shuffle=False, num_workers=nThreads)
    test_loader4 = torch.utils.data.DataLoader(test_data4, batch_size=256, shuffle=False, num_workers=nThreads)
    ...
    for epoch in range(epoch_num):
        ...
        if (epoch+1) % 1 == 0:
            acc0, acc5, test_loss = predict(test_loader0, model)
            acc1, acc5, test_loss = predict(test_loader1, model)
            acc2, acc5, test_loss = predict(test_loader2, model)
            acc3, acc5, test_loss = predict(test_loader3, model)
            acc4, acc5, test_loss = predict(test_loader4, model)
            macc, acc5, test_loss = predict(test_loader, model)
        # torch.save(model.state_dict(), account + '/ada_optimizer/model/model_%s_%s_%s_batch%d_5i_lr%f_seed%d.pth' % (dataset_name, model_name, opt_name, batch_size, lr, seed))
        torch.save(model.state_dict(), account + '/ada_optimizer/model/model_%s_%s_%s_batch%d_%de_lr%s_seed%d.pth' % (dataset_name, model_name, opt_name, batch_size, epoch_num, lr, seed))
        # torch.save(model.state_dict(), account + '/ada_optimizer/model/model_%s_%s_%s_batch%d_%de_lr%f_seed0.pth' % (dataset_name, model_name, opt_name, batch_size, epoch_num, lr))
        print('train_loss: %4f, test_loss: %4f' % (train_loss, test_loss))
        logger.record([acc0.item(), acc1.item(), acc2.item(), acc3.item(), acc4.item(), macc.item()])
        logger.log()
        logger.write_log()
        scheduler.step()
