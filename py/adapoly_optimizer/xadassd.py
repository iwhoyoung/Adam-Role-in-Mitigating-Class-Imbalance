...
class XAdaSSD(Optimizer):
    ...
    def adam(self,
        ...
        for i, param in enumerate(params):
            ...
            # avg_grad = state['exp_avg'].mul_(0.9).add_(torch.abs(grad)**1, alpha=(0.1))/(1 - 0.9 ** state['step'])
            ...
            # grad = grad * avg_grad
            # mean_exp_avg = state['mean_exp_avg'].mul_(0.999).add_(grad, alpha=(0.001))/(1 - 0.999 ** state['step'])
