...
class Adam_Sbn(Optimizer):
    ...
    def adam(self,
        ...
        for i, param in enumerate(params):
            ...
            step_size = lr / bias_correction1
            # if step % 200 == 0:
            #     print("step:",step,"  lr:",step_size,"  param:",torch.mean(torch.abs(param)),"  dyn:",torch.mean(torch.abs(exp_avg/denom*step_size*initial_norm[i])),"  denom:",denom,"  exp_avg:",torch.mean(exp_avg))
            # if torch.isnan(exp_avg/denom).any():
            #     print("step:",step,"  denom:",denom,"  exp_avg:",torch.isnan(exp_avg).any())
            #     raise Exception
            param.addcdiv_(exp_avg, denom, value=-step_size*initial_norm[i])

class SGD_Sbn(Optimizer):
    r"""Implements Adam algorithm.
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps,
                        weight_decay=weight_decay, amsgrad=amsgrad)
        self.weight = Parameter(torch.zeros(1,device='cuda'))
        super(SGD_Sbn, self).__init__(params, defaults)


    def __setstate__(self, state):
        super(SGD_Sbn, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault('amsgrad', False)
    ...
    def adam(self,
        ...
        for i, param in enumerate(params):
            ...
            # else:
            #     denom = (exp_avg_sq.sqrt() / math.sqrt(bias_correction2)).add_(eps)
            denom = torch.mean((grad*grad.conj())).sqrt().add_(eps)
            # step_size = lr / bias_correction1
            step_size = lr
            # if step % 200 == 0:
            #     print("step:",step,"  lr:",step_size,"  param:",torch.mean(torch.abs(param)),"  dyn:",torch.mean(torch.abs(exp_avg/denom*step_size*initial_norm[i])),"  denom:",denom,"  exp_avg:",torch.mean(exp_avg))
            ...
            #     print("step:",step,"  denom:",denom,"  exp_avg:",torch.isnan(exp_avg).any())
            #     raise Exception
            param.addcdiv_(grad, denom, value=-step_size*initial_norm[i])