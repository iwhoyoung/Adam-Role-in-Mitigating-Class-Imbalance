...
from vit import vit_base_cifar_patch4_32, vit_cifar_patch4_32, vit_cifar_patch4_32_depth4, vit_cifar_patch4_256, vit_custom_cifar_32, vit_small_cifar_patch4_32, vit_tiny_cifar_patch4_32
...
model_name = 'swint'# res18vconv13x3 vgg16bn vits
opt_name = 'dyngrad'#sgdwnormandsumadam sgdwredistribution8andnorm adacertaintywoadd
...
def train(train_loader, model, criterion, optimizer,mean_loss=100):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        optimizer.step()
        train_loss.append(loss.item())
    # optimizer.calculate_mean_std(account + '/ada_optimizer/submit/dynamics_%s_%s_%s_batch%d_%de_lr%f_seed%d' % (dataset_name, model_name, opt_name, batch_size, epoch_num, lr, seed))
    return np.mean(train_loss)
...
if __name__ == '__main__':
    ...
    # model = ResNetOrigin(BasicBlock, [2, 2, 2, 2],num_classes=10) #3 4 6 3
    # model = ResNetOrigin(Bottleneck, [3, 4, 6, 3],num_classes=10) #3 4 6 3
    # model = ResNetOrigin(Bottleneck, [3, 4, 23, 3],num_classes=100) #3 4 6 3
    # model = models.vgg16_bn(pretrained=False,num_classes=10) #3 4 6 3
    # model = models.vgg16(pretrained=False,num_classes=10) #3 4 6 3
    # model = models.densenet121(num_classes=10) #3 4 6 3
    model = vit_small_cifar_patch4_32(num_classes=10)
    # model = vit_base_cifar_patch4_32(num_classes=10)
    # model = swin_t_32(num_classes=10)
    # vit_model_paper = ['vit_small_patch16_224', 'vit_base_patch16_224', 'pit_b_224',
    #                'visformer_small', 'swin_tiny_patch4_window7_224']
    # model = timm.create_model(vit_model_paper[0], pretrained=False)
    ...
    # model.load_state_dict(torch.load(account + '/ada_optimizer/model/model_cifar100_vits_adacertainty_batch256_200e_lr0.001_seed0.pth'),strict=False)
    ...
    # cal_sim_norm_model(model.state_dict(), a,account + '/ada_optimizer/submit/sim_icml24_cifar10_resnet18vconv13x3_sgd_batch1024_5i200e_lr0')# model.cuda().state_dict()
    # record_param_name(model.state_dict(),account + '/ada_optimizer/submit/name_%s' % model_name,['running','num','mask'])
    # raise Exception
    # test_cuda()
    if server_mode:
        logger = Visualizer(['train loss', 'test loss', 'acc', 'acc5'], 2, log_path=account + '/ada_optimizer/log/',
         name="ada_optimizer_1",
    ...
    # train_data = Food101(root = account + "/assets/dataset/food101/food-101", split = "train")
    # test_data = Food101(root = account + "/assets/dataset/food101/food-101", split = "test")
